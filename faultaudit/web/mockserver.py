#!/usr/bin/env python3
"""FaultAuditAI — Standalone mock server (stdlib only, no dependencies).
...
Usage:
    python faultaudit/web/mockserver.py
...
# ── in-memory run store ────────────────────────────────────────────────────────
runs: dict[str, dict] = {}   # run_id -> {queue, approval_event, approval_data, report}

def utcnow_iso() -> str:
    return datetime.now(timezone.utc).isoformat()

def make_event(run_id: str, event_type: str, data: dict) -> dict:
    return {
        "run_id": run_id,
        "type": event_type,
        "data": data,
...
*Report generated by FaultAuditAI. For demo purposes only.*
"""

def run_audit_sequence(run_id: str):
    """Replay the scripted event sequence, pausing at approval gates."""
    q: queue.Queue = runs[run_id]["queue"]
    plan_gate: threading.Event = runs[run_id]["plan_gate"]
    action_gate: threading.Event = runs[run_id]["action_gate"]

    def send(event_type: str, data: dict, delay: float = 0.8):
        time.sleep(delay)
        q.put(make_event(run_id, event_type, data))
    ...
    approval = runs[run_id].get("plan_decision", {})
    ...
    decision = runs[run_id].get("action_decision", {})
    ...
    # 6. Done
    send("done", {"run_id": run_id}, delay=0.5)
    q.put(None)  # sentinel
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path == "/api/mission":
            self._post_mission(body)
        elif path.startswith("/api/approve/"):
            run_id = path.removeprefix("/api/approve/")
            self._post_approve(run_id, body)
        else:
            self._not_found()
    ...
    # ── handlers ─────────────────────────────────────────────────────────────
    def _post_mission(self, body: dict):
        ...
        t = threading.Thread(target=run_audit_sequence, args=(run_id,), daemon=True)
        t.start()

        self._json({"run_id": run_id})

    def _sse_stream(self, run_id: str):
    ...
    def _post_approve(self, run_id: str, body: dict):
        if run_id not in runs:
            self._error(404, "run not found")
            return
        ...
        gate = body.get("gate", "")
        if gate == "plan":
            runs[run_id]["plan_decision"] = body
            runs[run_id]["plan_gate"].set()
        elif gate == "action":
            runs[run_id]["action_decision"] = body
            runs[run_id]["action_gate"].set()
        else:
    ...
    def _get_report(self, run_id: str):
        if run_id not in runs:
            self._error(404, "run not found")
            return
...
if __name__ == "__main__":
    ...
    print(f"FaultAuditAI mock server running at http://localhost:{PORT}")
